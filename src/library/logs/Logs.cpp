...
{
    std::string mode = doc->get("RecordPosition");
	if( mode != "False" )
	{
		if( mode == "Precise" )
			_mode = Precise;
...
void Logs::BrainComplexityLog::init( TSimulation *sim, Document *doc )
{
	if( doc->get("RecordComplexity") )
	{
        ...
        _complexityType = (std::string)doc->get( "ComplexityType" );
		_recordBestRecent = doc->get( "RecordBrainBestRecent" );
...
void Logs::GenomeMetaLog::processEvent( const sim::SimInitedEvent &birth )
{
	...
	}
	{
		...
		// TODO: Migirieren
        SYSTEM(UTILS_PATH "cat run/genome/meta/genelayout.txt | " UTILS_PATH "sort -n > run/genome/meta/genelayout-sorted.txt");
	}
	{
...
void Logs::GenomeSubsetLog::init( TSimulation *sim, Document *doc )
{
	if( doc->get("GenomeSubsetLog").get("Record") )
	{
		...
		for( int i = 0; i < (int)geneNames.size(); i++ )
		{
			if( _geneIndexes[i] < 0 )
			{
                std::cerr << "Invalid gene name for GenomeSubsetLog: " << geneNames[i] << std::endl;
				exit( 1 );
			}
		}
        ...
        itfor( std::vector<std::string>, geneNames, it )
		{
...
{
    if( (std::string)doc->get("RecordSeparations") != "False" )
	{
        std::string mode = doc->get( "RecordSeparations" );
		if( mode == "Contact" )
		{
			_mode = Contact;
			initRecording( sim,
...
void Logs::SeparationLog::processEvent( const sim::StepEndEvent &e )
{
	...
	if( !_births.empty() )
	{
        itfor( std::list<agent *>, _births, it )
		{
			agent *a_born = *it;
