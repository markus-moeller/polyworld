...

/* --- EDL_LOADLIBRARY() INTERFACE LOADER --- */

#ifdef __WINDOWS__

/* dynamic linking for Win32 with edll */
...
static lt_module
sys_edll_open (loader_data, filename)
     lt_user_data loader_data;
     const char *filename;
{
  return edll_open (filename);
}

static int
sys_edll_close (loader_data, module)
     lt_user_data loader_data;
     lt_module module;
{
  int	      errors   = 0;

  if (edll_close(module) != 0)
    {
      LT_DLMUTEX_SETERROR (LT_DLSTRERROR (CANNOT_CLOSE));
      ++errors;
    }
...
static lt_ptr
sys_edll_sym (loader_data, module, symbol)
     lt_user_data loader_data;
     lt_module module;
     const char *symbol;
{
  lt_ptr      address  = edll_sym (module, symbol);
...
static struct lt_user_dlloader sys_edll = {
  0, sys_edll_open, sys_edll_close, sys_edll_sym, 0, 0
};
...
/* use the edll (see before) instead */
#ifdef UNDEFINED__WINDOWS__
...
lt_dlinit ()
{
  ...
  if (++initialized == 1)
	...
	errors += lt_dlloader_add (lt_dlloader_next (0), &sys_edll, "dlopen");
      }
#endif
#ifdef __BEOS__
