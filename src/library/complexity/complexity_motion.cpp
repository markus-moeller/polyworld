...
void MotionComplexity::computeEpochs( long begin,
									  ...
									  long epochlen )
{
    DataLibReader in( (std::string(path_run) + "/lifespans.txt").c_str() );
    ...
    typedef std::map<long, Agent> AgentMap;
...
#define IEPOCH(STEP) (STEP - begin) / epochlen;
            ...
            int iepoch_abegin = IEPOCH( std::max(a.begin, begin) );
            int iepoch_aend = IEPOCH( std::min( a.end, end) );
				 ...
				 iepoch++ )
			{
                ...
                a_epoch.begin = std::max( a.begin, epoch.begin );
                a_epoch.end = std::min( a.end, epoch.end );
