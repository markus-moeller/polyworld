...
void gsamp( gsl_matrix* m )
{
	...
	if( r < c )
	{
        std::cerr << "gsamp: There are more cols than rows.  gsamp() requires that each neuron be a column.  We could do a tranpose here and continue, but considering the rest of the code this situation should be impossible as the matrix is already aligned.  So, there's probably something deeply wrong -- exiting.";
		exit(1);
	}
...
void print_matrix_row( gsl_matrix* m, int row )
{
    ...
    std::cout << "Length of row " << row << ": " << row_length << std::endl;
    std::cout << "Values in row: ";

	for( int temp = 0; temp < row_length; temp++ )
        std::cout << gsl_vector_get( &row_i.vector, temp) << "  ";
    std::cout << std::endl;
...
void print_matrix_column( gsl_matrix* m, int col )
{
    ...
    std::cout << "Length of col: " << col << ": " << col_length << std::endl;
    std::cout << "Values in col: ";

	for(int temp = 0; temp < col_length; temp++ )
        std::cout << gsl_vector_get( &col_i.vector, temp) << "  ";
    std::cout << std::endl;
...
void print_matrix( gsl_matrix* m )
{
	...
	{
		for( int j = 0; j < end; j++ )
            std::cout << "[" << i << "," << j << "]: " << gsl_matrix_get( m, i, j ) << "\t" ;
        std::cout << std::endl;
	}
}
