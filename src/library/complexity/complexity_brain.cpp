    ...
    if( events )
    {
    	...
    	for( unsigned int i = 0; i < strlen( part ); i++ )
    	{
    		if( islower( part[i] ) )
    		{
    			...
    			{
                    std::cerr << "Error: too many filter events specified (" << __func__ << ")" << std::endl;
					exit( 1 );
    			}
    		}
    	}
...
//---------------------------------------------------------------------------
std::vector<std::string> get_list_of_brainfunction_logfiles( std::string directory_name )
{
	...
	{
        std::cerr << "Could not opendir('" << directory_name << "') -- Terminating." << std::endl;
		exit(1);
	}
...
//			cout << "***File: " << entry->d_name << "***" << endl;
	...
	{
        std::cerr << "Could not closedir('" << directory_name << "').  -- Terminating." << std::endl;
		exit(1);
	}
...
//---------------------------------------------------------------------------
std::vector<std::string> get_list_of_brainanatomy_logfiles( std::string directory_name )
{
...
*/
	...
	{
        std::cerr << "Could not open file '" << fname << "' for reading. -- Terminating." << std::endl;
		exit(1);
	}
...
//DEBUG	cout << "str_tline = " << str_tline;
    ...
    std::string str_fitness = str_tline.substr(str_tline.find("=",0)+1, str_tline.find(" ",0) );
...
*/
	...
	for(int i=0; i<numneu; i++)
	{
        ...
        std::string str_cijline = cijline;					//This sort of thing could probably be optimized, but I'm not sure how.
...
// 	printf( "\nmax_timesteps = %d\n", max_timesteps );
	...
	indexSpace = params.find( " ", 0 );
    std::string numineu = params.substr( 0, indexSpace );
	...
	{
		indexSpace = params.find( " ", 0 );
        std::string numoneu = params.substr( 0, indexSpace );
	...
	indexSpace = params.find( " ", 0 );
    std::string num_synapses = params.substr( 0, indexSpace );
	params.erase( 0, indexSpace + 1 );

	// Read birth time
	indexSpace = params.find( " ", 0 );
    std::string birth_time = params.substr( 0, indexSpace );
	params.erase( 0, indexSpace + 1 );
	if( agent_birth )
	...
	{
        std::cerr << "Warning: #lines (" << FileContents.size() << ") in brainFunction file '" << fname << "' is not an even multiple of #neurons (" << numcols << ").  brainFunction file may be corrupt." << std::endl;
	}
	...
	if( num_timesteps > 0 )
	{
		...
		{
            numrows = std::min( numrows, num_timesteps ); //
	...
	{
        numrows = std::min( numrows, max_timesteps );
...
// 	printf( "numrows = %d\n", numrows );
	...
	{
        std::cerr << "brainFunction file '" << fname << "' is corrupt; numcols=" << numcols << ", numrows=" << numrows << std::endl;
		activity = NULL;
		return activity;
	}
    ...
    std::vector<std::string>::iterator FileContents_it;
    std::vector<std::string>::iterator FileContents_begin;
    std::vector<std::string>::iterator FileContents_end;
