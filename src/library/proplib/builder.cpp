...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
Document *DocumentBuilder::buildDocument( const std::string &path )
{
	proplib::Parser parser;
    SyntaxNode *node = parser.parseDocument( path, new std::ifstream(path.c_str()) );
...
Document *DocumentBuilder::buildWorldfileDocument( SchemaDocument *schema, const std::string &path )
{
	...
	{
        std::string pathv2 = path + ".v2";
		WorldfileConverter::convertV1SyntaxToV2( path, pathv2 );

		proplib::Parser parser;
        SyntaxNode *node = parser.parseDocument( pathv2, new std::ifstream(pathv2.c_str()) );
...
SchemaDocument *DocumentBuilder::buildSchemaDocument( const std::string &path )
{
	proplib::Parser parser;
    SyntaxNode *node = parser.parseDocument( path, new std::ifstream(path.c_str()) );
    ...
    itfor( std::set<std::string>, SchemaDocument::standardTypes, it )
		enum_->addValue( *it );
...
SymbolPath *DocumentBuilder::buildSymbolPath( const std::string &text )
{
	proplib::Parser parser;
...
MetaProperty *DocumentBuilder::buildMetaProperty( DocumentLocation loc, Identifier id, const std::string &value )
{
...
Property *DocumentBuilder::buildProperty( DocumentLocation loc, Identifier id, const std::string &value )
{
	_doc = loc.getDocument();
	_path = _doc->getPath();
													  ...
													  SyntaxNode *nodeObject )
{
    ...
    itfor( std::vector<SyntaxNode *>, nodeObject->children, it )
	{
...
Property *DocumentBuilder::buildArrayProperty( DocumentLocation loc, Identifier id, SyntaxNode *nodeValue )
{
    ...
    itfor( std::vector<SyntaxNode *>, nodeValue->children[0]->children, it )
	{
		SyntaxNode *childNode = *it;
...
Enum *DocumentBuilder::buildEnum( SyntaxNode *node )
{
    ...
    itfor( std::vector<SyntaxNode *>, nodeValues->children, it )
	{
...
Expression *DocumentBuilder::buildExpression( SyntaxNode *node )
{
    ...
    itfor( std::vector<SyntaxNode *>, node->children, it )
	{
