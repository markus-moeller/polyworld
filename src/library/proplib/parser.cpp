...
Token::Token( Type type_, const char *typeName_, const std::string &text_ )
: type( type_ )
, typeName( typeName_ )
	, text( text_ )
	, next( NULL )
...
std::string Token::getDecorationString()
{
	assert( !isDecoration() );
...
std::string Token::toString( Token *start, Token *end, bool leadingDecoration )
{
    std::stringstream out;
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
Tokenizer::Tokenizer( std::string sourceName, std::istream *in )
...
Token *Tokenizer::__next()
{
	...
	if( !_in->good() )
	{
		...
		{
            std::cerr << "Failed reading from " << _sourceName << std::endl;
			exit( 1 );
		}
	}
	...
	if( c == '#' )
        ...
        return NEW_TOKEN( Misc, std::string("") + c );
}
...
Token *Tokenizer::parseComment()
{
	...
	if( _in->peek() == '*' )
	{
		...
		if( state != Complete )
		{
            std::cerr << _sourceName << ":" << linenoStart << ": Unterminated multi-line comment." << std::endl;
			exit( 1 );
		}
...
Token *Tokenizer::parseString()
{
	...
	if( state != Complete )
	{
        std::cerr << _sourceName << ":" << lineno << ": Unterminated string literal." << std::endl;
		exit( 1 );
	}
...
Token *Tokenizer::parseNumber()
{
	...
	while( _in->good() && (state != Complete) )
	{
		...
		if( c == '.' )
		{
			if( state == Dot )
			{
                std::cerr << _sourceName << ":" << _lineno << ": Unexpected '.'" << std::endl;
				exit( 1 );
			}
		...
		else if( c == 'l' )
		{
			if( state == Dot )
			{
                std::cerr << _sourceName << ":" << _lineno << ": Illegal combination of '.' and 'l' suffix" << std::endl;
				exit( 1 );
			}
			state = Complete;
		}
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
void SyntaxNode::dump( std::ostream &out, std::string indent )
{
	out << indent << typeName;

	if( children.empty() )
	{
        out << "('" << Token::toString( beginToken, endToken ) << "')" << std::endl;
	}
	else
	{
        out << "('" << beginToken->text << "' --> '" << endToken->text << "')" << std::endl;

		indent += "  ";
        itfor( std::vector<SyntaxNode *>, children, it )
		{
			(*it)->dump( out, indent );
		}
	}
...
SyntaxNode *Parser::parseDocument( std::string path, std::istream *in )
{
	init( path, in );
...
SyntaxNode *Parser::parseSymbolPath( std::string text )
{
    init( "<string>", new std::istringstream(text) );
...
SyntaxNode *Parser::parsePropertyValue( std::string text )
{
    init( "<string>", new std::istringstream(text) );
...
SyntaxNode *Parser::parseMetaPropertyValue( std::string text )
{
    init( "<string>", new std::istringstream(text) );
...
void Parser::init( std::string path, std::istream *in )
{
	_path = path;
...
void Parser::err( Token *tok, std::string message )
{
    std::cerr << _path << ":" << tok->lineno << " " << message << std::endl;
	exit( 1 );
}
...
Token *Parser::next( Token::Type required )
{
	...
	{
        err( tok, std::string("Unexpected '") + tok->text + "'" );
	}
	return tok;
}
...
SyntaxNode *Parser::parseExpression( bool parenDelimited )
{
	...
	if( tokStart == NULL )
        err( _currTok, std::string("Expecting scalar after '") + _currTok->text + "'" );
