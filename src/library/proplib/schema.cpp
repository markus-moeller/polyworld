...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
std::set<std::string> SchemaDocument::standardTypes;
...
void SchemaDocument::injectClasses( Property *prop_ )
{
	if( prop_->getType() == Node::Object )
	{
		...
		if( propType )
		{
            ...
            // Required statement failed: ssize_t(writelen) == write(writebuf, writelen)
            std::string type = *propType;
...
void SchemaDocument::normalize( ObjectProperty &propertySchema, Property &propertyValue )
{
    ...
    std::string type = propertySchema.get( "type" );
			...
			if( objectValue.getp(propertySchema.getName()) == NULL )
			{
				...
				if( propDefault == NULL )
				{
					...
					if( !propOptional || !(bool)*propOptional )
                        objectValue.err( std::string("Missing property ") + propertySchema.getName() );
				}
				else
				{
...
void SchemaDocument::validate( Property &propertyValue )
{
    ...
    std::string type = propertyValue.getSchema()->get( "type" );
...
{
    std::string type = schema.get( "type" );
	...
	itfor( PropertyMap, schema.props(), it )
	{
		...
		{																\
			...
			if( type == "Int" )											\
                ...
                attr.err( std::string("'") + attrName + "' is not valid for type " + type ); \
																		\
			if( !valid )												\
                value.err( (std::string)value + " " ERRDESC " " + attrName + " " + (std::string)attr ); \
		}
...
void SchemaDocument::validateEnum( Property &propertyValue )
{
    ...
    if( !enum_->contains((std::string)propertyValue) )
	{
	...
	itfor( PropertyMap, propertyValue.getSchema()->props(), it )
	{
		...
		{															            \
			bool valid = (int)propertyValue.props().size() OP (int)attr;        \
																		        \
			if( !valid )												        \
                propertyValue.err( std::string("Element count ")+ERRDESC+" "+attrName+" " + (std::string)attr ); \
		}
...
void SchemaDocument::validateCommonAttribute( Property &attr, Property &value )
{
	...
	{
		if( !(bool)attr )
            value.err( std::string("Failed assertion at ") + attr.getLocation().getDescription() );
	}
...
{
    std::string type = schema.get( "type" );
    std::string childName = pathElement->getText();
	...
	else if( type == "Object" )
	{
		...
		if( value.getp(childName) == NULL )
		{
			...
			if( defaultProperty == NULL )
                schema.err( std::string("No default for ") + childName );
...
void SchemaDocument::parseDefaults( Document *doc )
{
	...
	{
        std::string defaults = doc->getMeta("@defaults")->getValue();
        std::istringstream in( defaults );
		...
		while( !in.eof() )
		{
			...
			else if( isdigit(last) )
			{
                ...
                std::string prefix = value.substr( 0, numstr - value.c_str() );
...
Property *SchemaDocument::createDefault( Property &schema )
{
	...
	if( !propDefault )
	{
		...
		if( propDefaults )
		{
            itfor( std::list<std::string>, _defaults, it )
			{
