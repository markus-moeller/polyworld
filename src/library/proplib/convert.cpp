...
bool WorldfileConverter::isV1( std::string path )
{
    std::ifstream in( path.c_str() );
	return in.get() != '@';
}

void WorldfileConverter::convertV1SyntaxToV2( std::string pathIn, std::string pathOut )
{
    std::stack<bool> isObjectArray;

    Tokenizer tokenizer( pathIn, new std::ifstream(pathIn.c_str()) );

    std::ofstream out( pathOut.c_str() );
...
#define QUOTE( CONTAINER, PROPNAME )									\
	{																	\
		...
		if( prop )														\
		{																\
            std::string value = RAWVAL(prop);								\
            editor->set( prop, std::string("\"") + value + "\"" );			\
		}																\
	}
...
#define CONDITION_TO_DYN( CONTAINER, PROPNAME )							\
	...
	// ---
	{
...

		{
            ...
            typedef std::map<std::string,bool> RecordMap;
			RecordMap record;
