...
std::string DocumentLocation::getPath() const
{
	return _doc->getName();
}

std::string DocumentLocation::getDescription()
{
	char buf[1024 * 4];
	if( _lineno > 0 )
...
void DocumentLocation::err( std::string msg )
{
...
void DocumentLocation::warn( std::string msg )
{
    std::string desc = getDescription();
...
namespace proplib
{
	...
	{
        std::string apath = a.getPath();
        std::string bpath = b.getPath();
...
void Node::err( std::string message )
{
	_loc.err( message );
}

void Node::warn( std::string message )
{
	_loc.warn( message );
}

void Node::dump( std::ostream &out, std::string indent )
{
}
...
void Enum::addValue( const std::string &name )
{
	if( contains(name) )
        err( std::string("Duplicate name: ") + name );

	_values.insert( name );
}

bool Enum::contains( const std::string &name )
{
...
Enum *Property::getEnum( const std::string &name )
{
...
bool Property::isEnumValue( const std::string &name )
{
	itfor( EnumMap, _enums, it )
...
{
    return _schema && (std::string)_schema->get("type") == "String";
}
...
{
    err( std::string("Invalid request for property: '") + id.getName() );
	return *this;
}
...
__ScalarProperty::operator std::string()
{
	return getEvaledString();
}

void __ScalarProperty::dump( std::ostream &out, std::string indent )
{
    out << indent << getName() << " (schema=" << getSchema() << ")" << std::endl;
}

int __ScalarProperty::toInt()
{
	char *end;
    std::string evaled = getEvaledString();
...
{
    std::string value = getEvaledString();
...
std::string ConstScalarProperty::getEvaledString()
{
...
std::string RuntimeScalarProperty::getEvaledString()
{
...
std::string DynamicScalarProperty::getEvaledString()
{
...
void __ContainerProperty::add( Property *prop )
{
	if( getp(prop->getName()) != NULL )
	{
        ...
        prop->err( std::string("Duplicate property name '") + prop->getName() + "' (see " + getp(prop->getName())->getLocation().getDescription() + ")" );
	}
...
Property &__ContainerProperty::get( Identifier id )
{
	...
	if( !prop )
	{
        err( std::string("No such property: '") + id.getName() + "'" );
	}
	return *prop;
}
...
__ContainerProperty::operator std::string()
{
	err( "Expecting String" ); return "";
}

void __ContainerProperty::dump( std::ostream &out, std::string indent )
{
    out << indent << getName() << " (schema=" << getSchema() << ")" << std::endl;
    out << indent << "{" << std::endl;
...
Class *ObjectProperty::getClass( const std::string &name )
{
...
std::string MetaProperty::getValue()
{
	return _value;
}
...
std::vector<float> Document::variables;

Document::Document( std::string name, std::string path )
...
bool Document::hasMeta( std::string name )
{
	return getMeta( name ) != NULL;
}

MetaProperty *Document::getMeta( std::string name )
{
