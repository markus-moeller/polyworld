...
void DocumentEditor::setMeta( std::string name, std::string value )
{
...
void DocumentEditor::set( std::string symbolPathString, std::string valueString )
{
	proplib::DocumentBuilder builder;
	...
	{
		if( _schema )
            err( std::string("Cannot find ") + symbolPathString + " in " + _doc->getPath() + " or " + _schema->getPath() );
		else
            err( std::string("Cannot find ") + symbolPathString + " in " + _doc->getPath() );
	}

	if( sym.type != Symbol::Property )
	{
        err( std::string("Only properties can be edited. (") + symbolPathString + ")" );
	}
...
void DocumentEditor::set( Property *prop, std::string valueString )
{
	...
	{
        err( std::string("Runtime properties cannot be edited. (") + prop->getFullName(1) + ")" );
	}
