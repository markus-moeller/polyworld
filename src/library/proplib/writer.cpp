...
DocumentWriter::DocumentWriter( std::ostream &out )
: _out( out )
{
}
...
void DocumentWriter::sortMeta( Document *doc, std::vector<MetaProperty *> &result )
{
...
void DocumentWriter::sortChildren( __ContainerProperty *prop, std::vector<Property *> &result )
{
...
void DocumentWriter::sortAttributes( DynamicScalarProperty *prop, std::vector<DynamicScalarAttribute *> &result )
{
...
{
    std::vector<MetaProperty *> sorted;
	sortMeta( doc, sorted );

    itfor( std::vector<MetaProperty *>, sorted, it )
	{
...
void DocumentWriter::writeProperties( ObjectProperty *object )
{
    ...
    itfor( std::vector<Property *>, sorted, it )
		writeProperty( *it );
}
...
void DocumentWriter::writeProperty( Property *prop )
{
	...
	if( prop->getParent()->getType() != Node::Array )
	{
        ...
        std::string decoration = tokId->getDecorationString();
...
void DocumentWriter::writeDynamic( DynamicScalarProperty *prop )
{
    ...
    std::vector<DynamicScalarAttribute *> attrs;
	...
	else if( !attrs.empty() )
	{
        ...
        itfor( std::vector<DynamicScalarAttribute *>, attrs, it )
		{
...
void DocumentWriter::writeScalarNames( Property *prop, int depthStart )
{
	...
	case Node::Object:
		...
		break;
	case Node::Scalar:
        _out << prop->getFullName( depthStart ) << std::endl;
		break;
	default:
		assert( false );
	}
