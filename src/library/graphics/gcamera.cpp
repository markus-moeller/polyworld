...
void gcamera::print()
{
    std::cout << "For the camera named \"" << fName << "\"...\n";
    std::cout << "  fov = " << fFOV nl;
    std::cout << "  fAspect = " << fAspect nl;
    std::cout << "  fNear = " << fNear nl;
    std::cout << "  fFar = " << fFar nl;
    std::cout << "  position (x,y,z) = (" << fPosition[0] comma fPosition[1] comma fPosition[2] pnl;

    if (fUsingLookAt)
        std::cout << "  using LookAt, not heading, (x,y,z) = ("
             << fFixationPoint[0] comma fFixationPoint[1] comma fFixationPoint[2] pnl;
    else
        std::cout << "  using heading, not LookAt, (yaw,pitch,roll) = ("
             << fAngle[0] comma fAngle[1] comma fAngle[2] pnl;
             
    if (fFollowObject != NULL)
    {
        unsigned long long address = (unsigned long long) fFollowObject;
        const char* objname = fFollowObject->GetName();
        std::cout << "  attached to object at " << address
             << " named: \"" << objname qnl;
    }
    else
    {
        std::cout << "  not attached to any object" nl;
	}
}
//---------------------------------------------------------------------------
// gcamera::SetFog
...
//			cout << "Function is Exponential. Density: " << density << endl;
		...
		{
            std::cerr << "Do not know glFogFunction beginning with '" << function << "'" << std::endl;
		}
	}
