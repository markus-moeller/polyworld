...
void gobject::dump(std::ostream& out)
{
    if (fName)
        out << fName nl;
    else
...
void gobject::load(std::istream& in)
{
    if (fName)
        delete fName;
    fName = new char[256];
...
void gobject::print()
{
    std::cout << "For object named = \"" << fName << "\"...\n";
    std::cout << "  position = " << fPosition[0] << ", "
                            << fPosition[1] << ", "
                            << fPosition[2] nl;
    std::cout << "  fScale = " << fScale nl;
    std::cout << "  default color (r,g,b) = ("
         << fColor[0] comma fColor[1] comma fColor[2] pnl;
    std::cout << "  radius = " << fRadius nl;
    std::cout.flush();
}
