...
void EatStatistics::Init()
{
    ...
    average.ratioFailed = std::numeric_limits<float>::quiet_NaN();
    average.ratioFailedYaw = std::numeric_limits<float>::quiet_NaN();
    average.ratioFailedVel = std::numeric_limits<float>::quiet_NaN();
}
...
const float *EatStatistics::GetProperty( const std::string &name )
{
	if( name == "EatFailed" )
	{
