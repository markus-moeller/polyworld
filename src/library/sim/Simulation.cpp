...
static std::vector<SheetSynapseType> SheetSynapseTypes =
	{
...
long LIBRARY_SHARED TSimulation::fStep;
...
//---------------------------------------------------------------------------
TSimulation::TSimulation( std::string worldfilePath, proplib::ParameterMap parameters )
	:
		...
		agentPovRenderer(NULL)
{
	...
	proplib::Document *worldfile;
    {
		...
		{
            std::ofstream out( "run/converted.wf" );
	...
	if( fLockStepWithBirthsDeathsLog )
	{
        ...
        std::cout << "*** Running in LOCKSTEP MODE with file 'LOCKSTEP-BirthsDeaths.log' ***" << std::endl;

		if( (fLockstepFile = fopen("LOCKSTEP-BirthsDeaths.log", "r")) == NULL )
		{
            std::cerr << "ERROR/Init(): Could not open 'LOCKSTEP-BirthsDeaths.log' for reading. Exiting." << std::endl;
			exit(1);
		}
		...
		{
            std::cout << "ERROR/Init(): Did not find any data lines in 'LOCKSTEP-BirthsDeaths.log'.  Exiting." << std::endl;
			exit(1);
		}

        SYSTEM(UTILS_PATH "cp LOCKSTEP-BirthsDeaths.log run/" );    // copy the LOCKSTEP file into the run/ directory.
	...
	// ---
	{
        SYSTEM( (UTILS_PATH "cp " + worldfile->getPath() + " run/original.wf").c_str() );
        SYSTEM( (UTILS_PATH "cp " + schema->getPath() + " run/original.wfs").c_str() );

		{
            std::ofstream out( "run/normalized.wf" );
			proplib::DocumentWriter writer( out );
			writer.write( worldfile );
		}
...
TSimulation::~TSimulation()
{
	...
	{
        std::ofstream fout( "run/endStep.txt" );
        fout << fStep << std::endl;
		fout.close();
	}
}
...
//	printf( "%s: frame = %lu\n", __FUNCTION__, frame );
	...
	{
        std::cerr << "Population crash at step " << fStep << std::endl;
		End( "PopulationCrash" );
		return;
	}
...
// TSimulation::End
//---------------------------------------------------------------------------
void TSimulation::End( const std::string &reason )
{
	{
        std::ofstream fout( "run/endReason.txt" );
        fout << reason << std::endl;
		fout.close();
	}
...
//---------------------------------------------------------------------------
std::string TSimulation::EndAt( long timestep )
{
	if( timestep < fStep )
    ...
    std::cout << "End At " << timestep << std::endl;
...
void TSimulation::InitAgents()
{
	...
	for (id = 0; id < fNumDomains; id++)
	{
        ...
        int limit = std::min((fMaxNumAgents - (long)objectxsortedlist::gXSortedObjects.getCount(AGENTTYPE)), fDomains[id].initNumAgents);
									  ...
									  long numSeeded )
{
    ...
    const std::string &path = fSeedFilePaths[ numSeeded % fSeedFilePaths.size() ];
	AbstractFile *in = AbstractFile::open( path.c_str(), "r" );
	if( in == NULL )
	{
        std::cerr << "Could not open seed file " << path << std::endl;
		exit( 1 );
	}

    std::cout << "seeding agent #" << agentNumber << " genome from " << path << std::endl;
...
{
    std::ifstream in("genomeSeeds.txt");

	if( in.fail() )
	{
        std::cerr << "Could not open genomeSeeds.txt" << std::endl;
		exit( 1 );
	}

    makeDirs("run/genome");
    SYSTEM(UTILS_PATH "cp genomeSeeds.txt run/genome");
	...
	while( !in.eof() )
	{
		...
		if( strlen(buf) )
		{
            fSeedFilePaths.push_back( std::string(buf) );
		}
	}

	if( fSeedFilePaths.size() == 0 )
	{
        std::cerr << "genomeSeeds.txt is empty!" << std::endl;
		exit( 1 );
	}
}
...
{
    std::ifstream in("seedPositions.txt");

	if( in.fail() )
	{
        std::cerr << "Could not open seedPositions.txt" << std::endl;
		exit( 1 );
	}

    SYSTEM(UTILS_PATH "mkdir -p run/motion/position && " UTILS_PATH "cp seedPositions.txt run/motion/position" );
	...
	while( !in.eof() )
	{
		...
		struct local
		{
            static float read_dim( std::istream &in )
			{
				float val;
                std::string raw;
				in >> raw;
				...
				if( raw[0] == 'r' )
				{
                    std::string ratiostr = raw.substr(1);
	...
	if( fSeedPositions.size() == 0 )
	{
        std::cerr << "seedPositions.txt is empty!" << std::endl;
		exit( 1 );
	}
}
		...
		else
		{
			...
			else
			{
				...
				if( fRandomBirthLocation )
				{
                    ...
                    x = pwclamp( x, 0.01, globals::worldsize - 0.01 );
                    z = pwclamp( z, -globals::worldsize + 0.01, -0.01 );
				}
...
void TSimulation::Pickup( agent* c )
{
	...
	while( objectxsortedlist::gXSortedObjects.prevObj( fCarryObjects, (gobject**) &o ) )
	{
		...
		if( ( o->x() + o->radius() ) < ( c->x() - c->radius() ) )
		{
            ...
            if( ( o->x() + 2.0 * std::max( std::max( food::gMaxFoodRadius, agent::config.maxRadius ), brick::gBrickRadius ) ) < ( c->x() - c->radius() ) )
...
void TSimulation::Fitness( agent *c )
{
	...
	if( (fCurrentFittestCount < MAXFITNESSITEMS) || (c->HeuristicFitness() > fCurrentMaxFitness[fCurrentFittestCount-1]) )
	{
		...
		else	// must insert
		{
            ...
            for( int j = std::min( fCurrentFittestCount, MAXFITNESSITEMS-1 ); j > i; j-- )
			{
...
void TSimulation::CreateAgents( void )
{
    ...
    if (maxToCreate > 0)
    {
		...
		for (id = 0; id < fNumDomains; id++)
		{
            fDomains[id].numToCreate = std::max(0L, fDomains[id].minNumAgents - fDomains[id].numAgents);
...
void TSimulation::updateFittest( agent *c )
{
    ...
    fMaxFitness = std::max( cFitness, fMaxFitness );
...
// 	printf( "%s (beginning): %ld %g\n", __func__, c->Number(), c->Complexity() );
	...
	if( c->Alive() )
	{
        std::cerr << "Error: Simulation's AgentFitness(agent) function called while agent is still alive" nl;
		exit(1);
	}
...
void TSimulation::processWorldFile( proplib::Document *docWorldFile )
{
	...
	{
        std::string foodGrowthModel = (std::string) doc.get( "FoodGrowthModel" );
...
#undef __SET
	...
	{
        std::string fightMode = doc.get( "FightMode" );
		if( fightMode == "Normal" )
...
#undef __SET
	...
	// Process FoodTypes
	{
		...
		for( int ifoodType = 0; ifoodType < numFoodTypes; ifoodType++ )
		{
            ...
            std::string name = propFoodType.get( "Name" );
	...
	// Process AgentMetabolisms
	{
		...
		for( int iMetabolism = 0; iMetabolism < numMetabolisms; iMetabolism++ )
		{
            ...
            std::string name = propMetabolism.get( "Name" );
			if( name == "Null" )
			{
				char tmp[128];
            ...
            std::string carcassFoodTypeMode = propMetabolism.get( "CarcassFoodTypeMode" );
			...
			{
                std::string foodTypeName = propMetabolism.get( "CarcassFoodTypeName" );
	...
	{
        std::string mode = doc.get( "AgentMetabolismSelectionMode" );
		if( mode == "Gene" )
	...
	// Process Domains
	{
		...
		for( int id = 0; id < fNumDomains; id++ )
		{
			...
			// Process InitAgentsPatch
			{
				...
				{
                    std::string val = propPatch.get( "Shape" );
					if( val == "R" )
						shape = 0;
					else if( val == "E" )
						shape = 1;
				...
				{
                    std::string val = propPatch.get( "Distribution" );
					if( val == "U" )
						distribution = 0;
					else if( val == "L" )
						distribution = 1;
	...
	{
        std::string prop = doc.get( "SmiteMode" );
		if( prop == "O" )
			fSmiteMode = 'O';
		else if( prop == "R" )
			fSmiteMode = 'R';
    ...
    fComplexityType = (std::string)doc.get( "ComplexityType" );
    ...
    fFogFunction = ((std::string)doc.get( "FogFunction" ))[0];
...
void TSimulation::initLockstepMode()
{
    ...
    std::cout << "Due to running in LockStepWithBirthsDeathsLog mode, the following parameter values have been forcibly reset as indicated:" nl;
    std::cout << "  DieAtMaxAge" ses agent::config.dieAtMaxAge nl;
    std::cout << "  Eat2Energy" ses agent::config.eat2Energy nl;
    std::cout << "  Mate2Energy" ses agent::config.mate2Energy nl;
    std::cout << "  Fight2Energy" ses agent::config.fight2Energy nl;
    std::cout << "  MaxSizePenalty" ses agent::config.maxSizePenalty nl;
    std::cout << "  Speed2Energy" ses agent::config.speed2Energy nl;
    std::cout << "  Yaw2Energy" ses agent::config.yaw2Energy nl;
    std::cout << "  Light2Energy" ses agent::config.light2Energy nl;
    std::cout << "  Focus2Energy" ses agent::config.focus2Energy nl;
    std::cout << "  Pickup2Energy" ses agent::config.pickup2Energy nl;
    std::cout << "  Drop2Energy" ses agent::config.drop2Energy nl;
    std::cout << "  CarryAgent2Energy" ses agent::config.carryAgent2Energy nl;
    std::cout << "  CarryAgentSize2Energy" ses agent::config.carryAgentSize2Energy nl;
    std::cout << "  CarryFood2Energy" ses food::gCarryFood2Energy nl;
    std::cout << "  CarryBrick2Energy" ses brick::gCarryBrick2Energy nl;
    std::cout << "  FixedEnergyDrain" ses agent::config.fixedEnergyDrain nl;
    std::cout << "  NumDepletionSteps" ses fNumDepletionSteps nl;
    std::cout << "  MaxPopulationPenaltyFraction" ses fMaxPopulationPenaltyFraction nl;
    std::cout << "  ApplyLowPopulationAdvantage" ses fApplyLowPopulationAdvantage nl;
    std::cout << "  EnergyBasedPopulationControl" ses fEnergyBasedPopulationControl nl;
}
...
void TSimulation::initFitnessMode()
{
    ...
    std::cout << "Due to running as a steady-state GA with a fitness function, the following parameter values have been forcibly reset as indicated:" nl;
    std::cout << "  MinNumAgents" ses fMinNumAgents nl;
    std::cout << "  MaxNumAgents" ses fMaxNumAgents nl;
	// 		cout << "  ProbabilityOfMutatingSeeds" ses fProbabilityOfMutatingSeeds nl;
	//		cout << "  MateThreshold" ses fMateThreshold nl;
	for( int i = 0; i < fNumDomains; i++ )
	{
        std::cout << "  Domain " << i << ":" nl;
        std::cout << "    initNumAgents" ses fDomains[i].initNumAgents nl;
        std::cout << "    minNumAgents" ses fDomains[i].minNumAgents nl;
    ...
    std::cout << "  ApplyLowPopulationAdvantage" ses fApplyLowPopulationAdvantage nl;
    std::cout << "  EnergyBasedPopulationControl" ses fEnergyBasedPopulationControl nl;
    std::cout << "  EndOnPopulationCrash" ses fEndOnPopulationCrash nl;
}
...
void TSimulation::initAdaptivityMode()
{
    ...
    std::cout << "Due to running in adaptivity mode, the following parameter values have been forcibly reset as indicated:" nl;
    std::cout << "  MaxSteps" ses fMaxSteps nl;
    std::cout << "  MinNumAgents" ses fMinNumAgents nl;
    std::cout << "  MaxNumAgents" ses fMaxNumAgents nl;
    std::cout << "  NumberToSeed" ses fNumberToSeed nl;
	for( int i = 0; i < fNumDomains; i++ )
	{
        std::cout << "  Domain " << i << ":" nl;
        std::cout << "    minNumAgents" ses fDomains[i].minNumAgents nl;
        std::cout << "    maxNumAgents" ses fDomains[i].maxNumAgents nl;
        std::cout << "    numberToSeed" ses fDomains[i].numberToSeed nl;
	}
    std::cout << "  AllowBirths" ses fAllowBirths nl;
    std::cout << "  EndOnPopulationCrash" ses fEndOnPopulationCrash nl;
    std::cout << "  DieAtMaxAge" ses agent::config.dieAtMaxAge nl;
    std::cout << "  MaxSeedEnergy" ses agent::config.maxSeedEnergy nl;
    std::cout << "  RandomSeedEnergy" ses agent::config.randomSeedEnergy nl;
    std::cout << "  RandomSeedMateWait" ses agent::config.randomSeedMateWait nl;
    std::cout << "  NumDepletionSteps" ses fNumDepletionSteps nl;
    std::cout << "  MaxPopulationPenaltyFraction" ses fMaxPopulationPenaltyFraction nl;
    std::cout << "  ApplyLowPopulationAdvantage" ses fApplyLowPopulationAdvantage nl;
    std::cout << "  EnergyBasedPopulationControl" ses fEnergyBasedPopulationControl nl;
...
{
    std::filebuf fb;
    if (fb.open("pw.dump", std::ios_base::out) == 0)
    {
...
//	statusText.push_back( strdup( t ) );
	...
	sprintf( t, "Fittest =" );
    int fittestCount = std::min( 5, fFittest->size() );
	...
	// --- addStat()
	// ---
    std::function<void (const char *, Stat &)> addStat =
...
void TSimulation::SetNextLockstepEvent()
{
	if( !fLockStepWithBirthsDeathsLog )
	{
        ...
        std::cerr << "ERROR: You called SetNextLockstepEvent() and 'fLockStepWithBirthsDeathsLog' isn't set to true.  Though not fatal, it's certain that you didn't intend to do this.  Exiting." << std::endl;
		exit(1);
	}
	...
	if( fgets( LockstepLine, sizeof(LockstepLine), fLockstepFile ) )			// get the next event, if the LOCKSTEP-BirthsDeaths.log still has entries in it.
	{
		...
		do
		{
			...
			{
				fLockstepNumBirthsAtTimestep++;
                std::cerr << "t" << fLockstepTimestep << ": Warning: a CREATION event occured, but we're simply going to treat it as a random BIRTH." << std::endl;
			}
			else
			{
                std::cerr << "ERROR/SetNextLockstepEvent(): Currently only support events 'DEATH', 'BIRTH', and 'CREATION' events in the Lockstep file.  Exiting.";
                std::cerr << "Latest Event: '" << LockstepEvent << "'" << std::endl;
				exit(1);
			}
