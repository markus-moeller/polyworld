...
{
    return std::max(0, _end.a - _begin.a + 1) * std::max(0, _end.b - _begin.b + 1);
}
...
{
    currentSize.a = std::max( currentSize.a, _neuronSpacing.a );
    currentSize.b = std::max( currentSize.b, _neuronSpacing.b );

    fieldSize.a = std::max( fieldSize.a, other->_neuronSpacing.a );
    fieldSize.b = std::max( fieldSize.b, other->_neuronSpacing.b );
	...
	for( Vector2i currentNeuronIndex : currentNeurons )
	{
        ...
        constrainedReceptiveFieldNeurons._begin.a = std::max( allReceptiveFieldNeurons._begin.a,
														 otherNeurons._begin.a );
        constrainedReceptiveFieldNeurons._begin.b = std::max( allReceptiveFieldNeurons._begin.b,
														 otherNeurons._begin.b );
        constrainedReceptiveFieldNeurons._end.a = std::min( allReceptiveFieldNeurons._end.a,
													   otherNeurons._end.a );
        constrainedReceptiveFieldNeurons._end.b = std::min( allReceptiveFieldNeurons._end.b,
													   otherNeurons._end.b );
...
// Sheet::createNeurons
//---------------------------------------------------------------------------
void Sheet::createNeurons( std::function<void (Neuron*)> &neuronCreated )
{
								 ...
								 const Vector2f &size )
{
    ...
    result._begin.a = std::max( 0, (int)ceilf( (ul.a - _neuronInsets.a) / _neuronSpacing.a ) );
    result._end.a = std::min( _neuronCount.a - 1, (int)floorf( (lr.a - _neuronInsets.a) / _neuronSpacing.a ) );
    result._begin.b = std::max( 0, (int)ceilf( (ul.b - _neuronInsets.b) / _neuronSpacing.b ) );
    result._end.b = std::min( _neuronCount.b - 1, (int)floorf( (lr.b - _neuronInsets.b) / _neuronSpacing.b ) );
...
//---------------------------------------------------------------------------
Sheet *SheetsModel::createSheet( std::string name,
								 int id,
								 Sheet::Type type,
								 Orientation orientation,
								 float slot,
