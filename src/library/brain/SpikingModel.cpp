        ...
        for (k = 0; k < dims->numSynapses; k++)
        {
            ...
            else
            {
                if (learningrate >= 0.0f)  // excitatory
                    synapse[k].efficacy = std::max(0.0f, synapse[k].efficacy);
                if (learningrate < 0.0f)  // inhibitory
                    synapse[k].efficacy = std::min(-1.e-10f, synapse[k].efficacy);
            }
        }
...
*/
	...
	for (i = 0; i < dims->numOutputNeurons; i++)
	{
        neuron[i+dims->getFirstOutputNeuron()].maxfiringcount = std::max(outputNeuronFiringCounter[i], (int) neuron[i+dims->getFirstOutputNeuron()].maxfiringcount);
