...
{
	{
        std::string val = doc.get( "BrainArchitecture" );
		if( val == "Groups" )
	...
	{
        std::string val = doc.get( "NeuronModel" );
		if( val == "F" )
	...
	{
        std::string val = doc.get( "LearningMode" );
		if( val == "None" )
...
void Brain::init()
{
    Brain::config.retinaWidth = std::max(Brain::config.minWin, GroupsBrain::config.maxvisneurpergroup);
